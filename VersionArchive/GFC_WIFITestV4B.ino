...
void appendFlightData(fs::FS &fs, const char * path, const char * message){
  Serial.printf("Appending to file: %s\n", path);
...
void loop(){
    ...
    while (client.connected()) {
      if (client.available()) {
        ...
        if (currentLine.endsWith("GET /ARM")) {
          ...
          toggle(3);
          fileNum += 1;
          writeFile(SD, convertStringToChar("/Recording" + std::to_string(fileNum)+ ".txt"), "Temp, pres, alt");
  ...
  if (getState(3)) {
  string dataLog = "";
  appendFile(SD, "/Recording" + std::to_string(fileNum)+ ".txt", convertStringToChar((dataLog.c_str(std::to_string(bmp.temperature) + ", " + std::to_string(bmp.pressure) + ", " + std::to_string(bmp.readAltitude(seapressure))+"\n"))));
  //appendFile(SD, "/temp.txt", convertDoubleToChar(bmp.temperature));
  //appendFile(SD, "/temp.txt", "\n");
  // dtostrf(bmp.temperature, 7, 0, tempstring);
  //appendFile(SD, "/pres.txt", convertDoubleToChar(bmp.pressure));
  //appendFile(SD, "/pres.txt", "\n");
  // dtostrf(bmp.temperature, 7, 0, tempstring);
  //appendFile(SD, "/alt.txt", convertDoubleToChar(bmp.readAltitude(seapressure)));
